...
CREATE TABLE `clazz`  (
                          ...
                          `clazz_name` varchar(255) CHARACTER SET utf8mb4 NULL DEFAULT NULL,
...
) ENGINE = InnoDB AUTO_INCREMENT = 55 CHARACTER SET = utf8mb4 ROW_FORMAT = DYNAMIC;
...
CREATE TABLE `subject`  (
                            ...
                            `subject_name` varchar(255) CHARACTER SET utf8mb4 NOT NULL,
...
CREATE TABLE `test`  (
                         ...
                         `test_name` varchar(255) CHARACTER SET utf8mb4 NULL DEFAULT NULL,
...
) ENGINE = InnoDB AUTO_INCREMENT = 105 CHARACTER SET = utf8mb4 ROW_FORMAT = DYNAMIC;
...
) ENGINE = InnoDB AUTO_INCREMENT = 7283 CHARACTER SET = utf8mb4 ROW_FORMAT = DYNAMIC;
...
CREATE ALGORITHM = UNDEFINED SQL SECURITY DEFINER VIEW `compare_short_answer_with_reply` AS
SELECT
    `compare_short_answer`.`compare_id` AS `compare_id`,
    `compare_short_answer`.`compare_threshold` AS `compare_threshold`,
    `compare_short_answer`.`short_answer_id` AS `short_answer_id`,
    `short_answer`.`question_id` AS `question_id`,
    `question`.`question_title` AS `question_title`,
    `short_answer`.`test_id` AS `test_id`,
    `compare_short_answer`.`reply1_id` AS `reply1_id`,
    `test_history_question_reply_1`.`reply` AS `reply1_text`,
    (
        SELECT COUNT(*) + 1
        FROM `test_history` `test_history_ranking1`
        WHERE
                `test_history_ranking1`.`test_id` = `short_answer`.`test_id` AND
                `test_history_ranking1`.`student_id` = `user1`.`user_id` AND
                `test_history_ranking1`.`history_id` < `test_history_1`.`history_id`
    ) AS `reply1_ranking`,
    `user1`.`user_id` AS `user_id`,
    `user1`.`username` AS `username`,
    `compare_short_answer`.`reply2_id` AS `reply2_id`,
    `test_history_question_reply_2`.`reply` AS `reply2_text`,
    (
        SELECT COUNT(*) + 1
        FROM `test_history` `test_history_ranking2`
        WHERE
                `test_history_ranking2`.`test_id` = `short_answer`.`test_id` AND
                `test_history_ranking2`.`student_id` = `user2`.`user_id` AND
                `test_history_ranking2`.`history_id` < `test_history_2`.`history_id`
    ) AS `reply2_ranking`,
    `user2`.`user_id` AS `compare_user_id`,
    `user2`.`username` AS `compare_username`
FROM
    `compare_short_answer`
        JOIN `short_answer` ON `short_answer`.`short_answer_id` = `compare_short_answer`.`short_answer_id`
        JOIN `question` ON `short_answer`.`question_id` = `question`.`question_id`
        JOIN `test_history_question_reply` `test_history_question_reply_1` ON `test_history_question_reply_1`.`reply_id` = `compare_short_answer`.`reply1_id`
        JOIN `test_history` `test_history_1` ON `test_history_1`.`history_id` = `test_history_question_reply_1`.`history_id`
        JOIN `user` `user1` ON `user1`.`user_id` = `test_history_1`.`student_id`
        JOIN `test_history_question_reply` `test_history_question_reply_2` ON `test_history_question_reply_2`.`reply_id` = `compare_short_answer`.`reply2_id`
        JOIN `test_history` `test_history_2` ON `test_history_2`.`history_id` = `test_history_question_reply_2`.`history_id`
        JOIN `user` `user2` ON `user2`.`user_id` = `test_history_2`.`student_id`;
