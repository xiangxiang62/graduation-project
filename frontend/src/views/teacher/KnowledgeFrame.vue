...
export default {
  ...
  methods: {
    toSubjectInfo() {
      console.log("toSubjectInfo method called");
    ...
    getNextChapter() {
      ...
      let lastItem = this.treeData[this.treeData.length - 1];

      if (!lastItem || !lastItem.label) {
        return 1; // 如果最后一个元素不存在或者没有 'label' 属性，返回 1
      }

      let label = lastItem.label;
    ...
    save(treeList) {
      ...
      this.$axios
          ...
          .then((response) => {
            console.log("Save successful", response.data); // 打印请求成功时的返回数据
            this.$message({
          ...
          .catch(function (error) {
            console.error("Error saving data:", error); // 打印请求失败时的错误信息
